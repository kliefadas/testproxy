...
module.exports = class ProxyProvider {
    ...
    _getAvailableProxies() {
        this._logger.info('Retrieve proxy start1');
        return new Promise((resolve) => {
            let proxies = [];

            const request = this.getProxyLists().getProxies({
                protocols: ['http'],
              sourcesWhiteList: ['gatherproxy'],
              countries: ['pl', 'cz','de','gb'],
                sourcesBlackList: ['bitproxies', 'kingproxies', 'premproxy','sockslist','coolproxyç','coolproxy','new-net-time','proxy-list-org',
                'proxydb','proxylisten','xroxy']
            });

            request.on('data', (newProxies) => {
                console.log(newProxies.length)
                this._logger.progress();
